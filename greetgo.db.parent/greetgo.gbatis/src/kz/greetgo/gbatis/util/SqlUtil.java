...
public class SqlUtil {
  ...
  public static Object forSql(Object value) {
    ...
    if (value.getClass().isEnum() || value instanceof Enum) {
      try {
        Method method = Enum.class.getMethod("name", new Class<?>[] {});
  ...
  public static Long asLong(Object fieldValue) {
    ...
    if (fieldValue instanceof Integer) return Long.valueOf(((Integer)fieldValue).intValue());
  ...
  private static Object defaultPrimitiveValue(Class<?> pr) {
    if (Integer.TYPE.equals(pr)) return Integer.valueOf(0);
    if (Long.TYPE.equals(pr)) return Long.valueOf(0);
    if (Boolean.TYPE.equals(pr)) return Boolean.FALSE;
    if (Short.TYPE.equals(pr)) return Short.valueOf((short)0);
    if (Float.TYPE.equals(pr)) return Float.valueOf(0);
    if (Double.TYPE.equals(pr)) return Double.valueOf(0);
    if (Byte.TYPE.equals(pr)) return Byte.valueOf((byte)0);
    if (Character.TYPE.equals(pr)) return Character.valueOf((char)0);
  ...
  private static Object sqlValueConvertTo(Object value, Class<?> toType) {
    ...
    if (toType.isAssignableFrom(Double.class) || toType == Double.TYPE) {
      ...
      if (value instanceof Float) return new Double(((Float)value).toString());
