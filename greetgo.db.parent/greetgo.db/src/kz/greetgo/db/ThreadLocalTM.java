...
class ThreadLocalTM {
  ...
  public void downLevel(Throwable throwable) {
    ...
    if (noAnyTransactional() && lastCallMeta.isTransactional()) {
      if (throwable == null) {
        ...
        closeType = lastCallMeta.needToCommit(throwable) ? CloseType.COMMIT_ON_CLOSE : CloseType.ROLLBACK_ON_CLOSE;
      }
    }
  ...
  private void doConnectionClose(Connection connection, CloseType closeType, List<Throwable> throwList) {
    try {
      if (closeType != null) {
      ...
      if (throwList == null) exceptionCatcher.catchException(e);
  ...
  private static void applyIsolationLevel(Connection connection, IsolationLevel isolationLevel) throws SQLException {
